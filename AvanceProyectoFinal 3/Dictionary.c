...
int main()
{
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        while (strcmp(current->key, NIL_STRING) != 0)
        {
            if (strcmp(w1, current->key) <= 0)
            {
        ...
        struct nodeRBTree *start = current->parent;
        while (start != NULL && strcmp(start->key, w1) < 0)
        {
            start = start->parent;
        }
        ...
        struct nodeRBTree *end = current->parent;
        while (end != NULL && strcmp(end->key, w2) > 0)
        {
            end = end->parent;
        }

        if (start != NULL && end != NULL &&
            strcmp(start->key, w2) <= 0 && strcmp(end->key, w1) >= 0)
        {
            result = end->position - start->position + 1;
        }
